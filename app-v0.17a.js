...
async function loadMtcYear4Classes() {
  if (!mtcReportClass || !generateMtcReportButton) return;

  const session = getStoredSession();

  if (!session) {
    mtcReportClass.innerHTML = `<option value="">Sign in to load classes</option>`;
    generateMtcReportButton.disabled = true;
    return;
  }

  mtcReportClass.innerHTML = `<option value="">Loading Year 4 classes…</option>`;
  generateMtcReportButton.disabled = true;
  ...
  try {
    ...
    if (!json.success) {
      throw new Error(json.error || "Year 4 classes could not be loaded.");
    }

    const classes = Array.isArray(json.classes)
      ? json.classes.map(value => String(value || "").trim()).filter(Boolean)
      : [];

    if (!classes.length) {
      mtcReportClass.innerHTML = `<option value="">No active Year 4 classes found</option>`;
      generateMtcReportButton.disabled = true;
      return;
    }
    ...
    console.error(error);
    mtcReportClass.innerHTML = `<option value="">Unable to load Year 4 classes</option>`;
    generateMtcReportButton.disabled = true;

    if (mtcReportResult) {
      mtcReportResult.textContent =
        error.message || "Year 4 classes could not be loaded.";
    }
  }
}
